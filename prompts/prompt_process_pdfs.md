...
7. Update the CSV: set `has_pdf` or `has_si` to `TRUE` for that row. If you are creating a new row, also populate the `date` column with the most precise date available (`YYYY-MM-DD`, else `YYYY-MM`, else `YYYY`). The `date` column is the source of truth for both year and within-year ordering — there is no separate `year` column. To find the exact publication date, query the CrossRef API: `https://api.crossref.org/works/{doi}` — look for `published-online` (preferred) or `published-print` date-parts in the JSON response. If CrossRef lacks a day, try `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&id={pmid}&retmode=json` (get the PMID first via `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term={doi}[doi]&retmode=json`). For preprints, use the server's posted date.
